...
TestData = TestData(:,1:500:end);  % sample around 1000 test samples  
TestLabels = TestLabels(1:500:end); 
% ===========================================================
TrnData=imnoise(TrnData,'gaussian',0,0.2);
TestData=imnoise(TestData,'gaussian',0,0.2);
...
fprintf('\n ====== PCANet Training ======= \n')
% convert columns in TrnData to cells 
TrnData_ImgCell = mat2imgcell(TrnData,ImgSize,ImgSize,ImgFormat); 
% fprintf('size of TrnData is %dx%dx%d\n',size(TrnData,1),size(TrnData,2),size(TrnData,3))
% fprintf('size of TrnData_ImgCell is %dx%dx%d\n',size(TrnData_ImgCell,1),size(TrnData_ImgCell,2),size(TrnData_ImgCell,3))
clear TrnData; 
tic;

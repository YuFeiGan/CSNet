...
V = cell(PCANet.NumStages,1); %filterÈÝÆ÷  2x49x8
OutImg = InImg; 
ImgIdx = (1:NumImg)';
clear InImg; 
%% ÑµÁ·filter
for stage = 1:PCANet.NumStages
    ...
    V{stage} = PCA_FilterBank(OutImg, PCANet.PatchSize, PCANet.NumFilters(stage),PCANet.scale); 
    ...
    if stage ~= PCANet.NumStages 
        [OutImg ImgIdx] = PCA_output(OutImg, ImgIdx, ...
            PCANet.PatchSize, PCANet.NumFilters(stage), V{stage}, PCANet.scale);  
    end
end
...
if IdtExt == 1 % enable feature extraction
    ...
    for idx = 1:NumImg
        ...
        [OutImg_i ImgIdx_i] = PCA_output(OutImg(OutImgIndex), ones(sum(OutImgIndex),1),...
            PCANet.PatchSize, PCANet.NumFilters(end), V{end}, PCANet.scale);  
